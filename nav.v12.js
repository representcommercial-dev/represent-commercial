// Self-contained mobile navigation — v12.
...
(function () {
  ...
  st.textContent =
    ...
    '.site-nav--mobile .nav-drop__menu a{display:block;padding:11px 24px 11px 40px;font-size:14px;color:rgba(255,255,255,0.55)}';
  document.head.appendChild(st);

  // ── group secondary pages under their parent link (desktop dropdowns) ──
  function makeDrop(parentHref, items) {
    var trigger = nav.querySelector('a[href="' + parentHref + '"]');
    ...
    items.forEach(function (it) {
      ...
      a.href = it[1];
      a.textContent = it[0];
      if (decodeURIComponent(it[1]) === decodeURIComponent(here)) a.className = 'is-active';
      menu.appendChild(a);
    });
    wrap.appendChild(menu);
  }
  makeDrop('services.html', [['Commercial Property Management', 'services.html'], ['Commercial Property Leasing', 'leasing.html'], ['Locations', 'locations.html']]);
  makeDrop('insights.html', [['Market Intelligence', 'insights.html'], ['Case Studies', 'case-studies.html'], ['Tools & Calculators', 'tools.html']]);
  makeDrop('about.html', [['About', 'about.html'], ['Referrals', 'referrals.html'], ['Contact', 'contact.html']]);
  ...
  function paintMenu() {
    ...
    Array.prototype.forEach.call(nav.querySelectorAll('a'), function (a) {
      ...
      a.style.cssText = isCta
        ? 'display:block;margin:12px 24px 4px;text-align:center;'
        : 'display:block;padding:14px 24px;font-size:16px;color:rgba(255,255,255,.82);border:0;border-radius:0;text-align:left;';
    });
  }
